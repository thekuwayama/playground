...
class UserTest < ActiveSupport::TestCase
  ...
  test "User.new_token should return random string of which length is 22" do
    token = User.new_token
    assert_equal token.class, String
    assert_equal token.length, 22
  end

  test "User.digest(string) should return hash of which length is 60" do
    digest = User.digest("test")
